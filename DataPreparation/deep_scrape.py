...
def gettext(x):
    ...
    a = ''.join(result).strip()
    key = re.sub('([a-zA-Z])', lambda x: x.groups()[0].upper(), a, 1)
    return key + ". "
...
def vraps(num):
    ...
    # get starter
    links = set(starter())
    # iterate over desired recursions
    for k in range(num):
...
def treatment(text):
    # joins the text with a space, to avoid sentences without a space after a period.
    for i in text:
        i.strip()
    a = ''.join(text)
    ...
    b = re.split(r'(?<=[^A-ZÆØÅ0-9][.?!:]) +(?=[A-ZÆØÅ])', a)
    return b
...
def getter(num):
    ...
    text = []
    # iterate through links to get text and collect it in one big list
    for i in links:
        res = gettext(i)
        text.extend(res)
...
def importer(num):
    res = getter(num)
    with open("output.csv", 'w', encoding="utf-8") as resultFile:
