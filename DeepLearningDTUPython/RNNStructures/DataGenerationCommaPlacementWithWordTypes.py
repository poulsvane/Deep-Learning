...
with open("Data\onehot_encoded.csv", mode='r') as oneHotEncoded:    
    ...
    for row in reader:
        ...
        for number in re.findall(pattern, mads):
            s = s + str(number[0])+" "
            typeString = typeString + str(number[1])+" "
        sen_len = len(s.split(' '))
        if commaEncodingString in s and sen_len<30:
            #print(s)
            listOfSentences.append(s)
            listOfSentencesTypes.append(typeString)

        
print("sentence number 6: ", len(listOfSentences))
print(listOfSentences[2])
print(listOfSentencesTypes[2])
...
def get_batch_comma(batch_size = 100, indices_of_interest = [0,1,2]):
    ...
    for i in indices_of_interest:
        ...
        listOfIntInputTypes = listOfSentencesTypes[i].split(' ')

        #print("input types: ", listOfIntInputTypes)
        #print("input as list:", listOfIntTarget)        

        text_target_in = start_character + text_target
        
        text_target_out = str(listOfIntTarget.index(commaEncodingString))+stop_character #text_target[0] + stop_character
        ...
        int_target_in_types =[int(digit) for digit in listOfIntInputTypes if digit is not '']
        ...
        int_target_in.remove(commaEncoding)        
        ...
        int_target_in_types.remove(int_target_in_types[indexOfComma])
        ...
        int_inputs_word_types.append(int_target_in_types)
...
#     targets_seqlen = np.asarray(map(len, int_targets_in))
    ...
    targets_out_seqlen = np.asarray([len(i) for i in int_targets_out])
        # The encoded inputs
        ...
        # the length of the target input sequence
        # the length of the target output sequence
        # the important parts of the output sequence
        # text representations of the problem - purely for debugging and "human leisureability"
        # the word types that corresponds to the input sequence
    return  inputs.astype('int32'), \
            inputsTypes.astype('int32'), \
