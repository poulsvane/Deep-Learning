...
def get_batch(batch_size=100, min_digits = 3, max_digits=3):
    ...
    for i in range(batch_size):
        ...
        #for random position of comma: comma_position = np.random.randint(0,tar_len)
        text_target = inp_str = "".join(map(str,np.random.randint(0,10,tar_len)))
        i = text_target.index('6') if '6' in text_target else None
        j = text_target.index('4') if '4' in text_target else None
        #place comma where the first 4 or 6 digit appears.
        smallestIndex = 1
        if i is not None:
            if j is not None:
                if i<j:
            ...
            smallestIndex = max_digits
        
        text_target_in = start_character + text_target[:smallestIndex]+','+text_target[smallestIndex:]
        
        text_target_out = str(text_target_in.index(special_character))+stop_character #text_target[0] + stop_character
